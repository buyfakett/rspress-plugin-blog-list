...
export function useTiltEffect(
    selector: string,
    { maxDegree = DEFAULT_MAX_DEGREE, disabled = false }: { maxDegree?: number; disabled?: boolean } = {},
) {
    useEffect(() => {
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            item.style.setProperty('--rs-blog-card-pointer-x', `${e.clientX - rect.left}px`);
            item.style.setProperty('--rs-blog-card-pointer-y', `${e.clientY - rect.top}px`);
        };
